(() => {
  const state = {
    ...
    sma20Series: null,
    sma50Series: null,
    ema200Series: null,            // <- restored long EMA
    trendSeries: null,
  ...
  function resizeChart() {
    ...
    if (hHost && state.haco.main) state.haco.main.resize(hHost.clientWidth || 0, hHost.clientHeight || 420);
  ...
  // ---------- Helpers ----------
  // tolerant alignment of signal points to candle time axis
  function buildBarsAligned(candles, points) {
    const out = [];
    const pts = (points || []).map(p => ({ t: Number(p.time), v: p.value })).sort((a,b)=>a.t-b.t);
    let i = 0;
    const TOL = 2*60*60;                    // 2h tolerance for intraday
    const dayKey = t => Math.floor(t / 86400);

    for (const c of (candles || [])) {
      const ct = Number(c.time);
      const cday = dayKey(ct);
      while (i+1 < pts.length && pts[i+1].t <= ct) i++;
      const cand = [pts[i], pts[i+1]].filter(Boolean);

      let v = null;
      if (cand.length) {
        const sameDay = cand.find(p => dayKey(p.t) === cday);
        if (sameDay) v = sameDay.v;
        else {
          const nearest = cand.reduce((a,b) => (Math.abs(a.t-ct) < Math.abs(b.t-ct) ? a : b));
          if (Math.abs(nearest.t - ct) <= TOL) v = nearest.v;
        }
      }
      ...
      out.push({ time: ct, value: v, color });
    }
    return out;
  }

  // mini-chart factory (passive: no local zoom/scroll)
  ...
  // one-way, time-based sync (master -> slaves)
  ...
  function ema(values, period) {
    ...
    for (let i=0;i<values.length;i++){
      const v = values[i];
      if (i < period) { seed += v; if (i === period-1) out[i] = seed/period; }
      else out[i] = (v - out[i-1]) * k + out[i-1];
    }
    return out;
  }
  ...
  function ensureHacoMinis() {
    ...
    if (!state.haco.main && window.HACO?.chart) state.haco.main = window.HACO.chart;
  ...
  function renderHacoSection(chartPayload) {
    // make sure HACO section chart/minis exist
    ensureHacoCharts();
    ensureHacoMinis();
    if (!chartPayload) return;

    // align minis to the same candle time axis
  ...
  function ensureChart() {
    ...
    if (!state.chart) {
      ...
      state.ema200Series = state.chart.addLineSeries({ color: '#f59e0b', lineWidth: 2 }); // 200 EMA
      state.trendSeries  = state.chart.addLineSeries({ color: '#ab47bc', lineWidth: 1, lineStyle: LightweightCharts.LineStyle.Dotted });
  ...
  function renderChart(chartPayload) {
    ...
    // minis aligned to candle times (tolerant)
    const hacoBars   = buildBarsAligned(candles, chartPayload.haco   || []);
    const hacoltBars = buildBarsAligned(candles, chartPayload.hacolt || []);
    ...
    // 200 EMA (computed here)
    const closes = candles.map(c => c.close);
    const ema200 = ema(closes, 200);
    state.ema200Series.setData(
      candles.map((c,i) => (ema200[i] != null ? { time: c.time, value: ema200[i] } : null)).filter(Boolean)
    );
  ...
  function renderPanels(panels) {
    const grid = el('panels-grid'); if (!grid) return;
    grid.innerHTML = '';
    (panels || []).forEach((panel) => {
      ...
      card.innerHTML = `
        <header><h4>${panel.title}</h4><span class="badge">${Math.round(panel.score)}</span></header>
  ...
  function renderEntries(entries) {
    const list = el('entries-list'); if (!list) return;
    list.innerHTML = '';
    (entries || []).forEach((entry) => {
      const li = document.createElement('li');
      const confidence = entry.confidence != null ? `${Math.round(entry.confidence)}%` : '—';
      li.innerHTML = `<strong>${entry.type ?? 'neutral'}</strong><span>${entry.summary ?? ''}</span><em>${confidence}</em>`;
      list.appendChild(li);
    });
  }

  function renderExits(exits) {
    const container = el('exits-summary'); if (!container) return;
    container.innerHTML = '';
    if (!exits || !exits.levels) { container.textContent = exits?.reason || 'No exit data available.'; return; }
    const table = document.createElement('table'); table.className = 'levels-table';
    const header = document.createElement('tr'); header.innerHTML = '<th>Risk</th><th>Level</th>'; table.appendChild(header);
    Object.entries(exits.levels).forEach(([risk, value]) => {
      const row = document.createElement('tr'); row.innerHTML = `<td>${risk}</td><td>${value ?? '—'}</td>`; table.appendChild(row);
    });
    container.appendChild(table);
    const reason = document.createElement('p'); reason.className = 'muted'; reason.textContent = exits.reason ?? ''; container.appendChild(reason);
  }
  ...
  function renderAdvancedTabs(tabs) {
    ...
    state.tabs = tabs || [];
    if (!state.tabs.length) { content.textContent = 'No advanced notes for this mode.'; return; }
    state.tabs.forEach((tab, index) => {
      const btn = document.createElement('button'); btn.type = 'button';
      btn.textContent = tab.title || tab.id; btn.dataset.tabId = tab.id;
      if (index === 0) { btn.classList.add('active'); state.activeTab = tab.id; }
  ...
  function activateTab(tabId) {
    const content = el('advanced-tab-content'); const nav = el('advanced-tabs-nav');
    if (!content || !nav) return;
    state.activeTab = tabId;
    [...nav.querySelectorAll('button')].forEach((btn) => btn.classList.toggle('active', btn.dataset.tabId === tabId));
    const tab = state.tabs.find(t => t.id === tabId) || state.tabs[0]; content.innerHTML = '';
    if (!tab) return;
    const items = Array.isArray(tab.content) ? tab.content : [tab.content];
    const list = document.createElement('ul');
    items.filter(Boolean).forEach((item) => { const li = document.createElement('li'); li.textContent = item; list.appendChild(li); });
    content.appendChild(list);
  }

  function renderMindset(mindset) {
    const taglineEl = el('mindset-tagline'); const focusEl = el('mindset-focus');
    if (taglineEl) taglineEl.textContent = mindset?.tagline || '';
    if (focusEl) {
      focusEl.innerHTML = '';
      (mindset?.focus || []).forEach((point) => { const li = document.createElement('li'); li.textContent = point; focusEl.appendChild(li); });
    }
  }

  function renderWatchlist(watchlist) {
    const list = el('watchlist-list'); if (!list) return;
    list.innerHTML = '';
    (watchlist || []).forEach((symbol) => {
      const li = document.createElement('li');
      const btn = document.createElement('button'); btn.type = 'button'; btn.textContent = symbol;
      btn.addEventListener('click', () => { const symbolInput = el('symbol'); if (symbolInput) symbolInput.value = symbol; runSignal(); });
      li.appendChild(btn); list.appendChild(li);
    });
  }

  function renderModes(modes, selected) {
    const select = el('mode-select'); if (!select) return;
    const normalized = (modes || []).map((mode) => mode.toLowerCase());
    const current = new Set(); select.innerHTML = '';
    normalized.forEach((mode) => {
      const option = document.createElement('option'); option.value = mode;
      option.textContent = mode.charAt(0).toUpperCase() + mode.slice(1);
      if (mode === selected) option.selected = true; current.add(mode);
      select.appendChild(option);
    });
    if (!current.has(selected) || !selected) {
      const fallback = document.createElement('option'); fallback.value = 'swing'; fallback.textContent = 'Swing'; fallback.selected = true; select.appendChild(fallback);
    }
    state.lastMode = select.value || 'swing';
  }

  function applyLegacySections(data) {
    const newsEl = el('news-output'); const techEl = el('tech-output');
    if (newsEl) { newsEl.className = ''; const score = data?.news?.score ?? 0; newsEl.textContent = `Score: ${score}`;
      if (score > 0) newsEl.classList.add('bullish'); else if (score < 0) newsEl.classList.add('bearish'); }
    if (techEl) { techEl.className = ''; const signal = data?.technical?.signal ?? 'none'; techEl.textContent = signal;
      if (signal === 'bullish') techEl.classList.add('bullish'); else if (signal === 'bearish') techEl.classList.add('bearish'); }
  }

  // ---------- Fetch/boot ----------
  async function fetchSignal() {
    const symbolInput = el('symbol'); const modeSelect = el('mode-select');
    if (!symbolInput) return null;
    const sym = symbolInput.value.trim().toUpperCase(); if (!sym) return null;
    ...
    try {
      ...
      renderChart(data.chart);            // top
      renderHacoSection(data.chart);      // haco section
      renderChecklist(data.panels, data.readiness, data.meta);

      // subtitle chips near top chart
      ...
      let hacoltChip = (meta.hacolt_now === 100) ? 'LT Up' : (meta.hacolt_now === 0) ? 'LT Down' : '';
      ...
      if (chartCol && !document.getElementById('chart-subtitle')) {
        const h = document.createElement('div'); h.style.margin = '6px 0 8px'; h.id = 'chart-subtitle'; h.textContent = subtitle.textContent;
        chartCol.prepend(h);
  ...
  function renderChecklist(panels, readiness, meta) {
    const container = document.getElementById('advanced-tab-content'); if (!container) return;
    const byId = Object.fromEntries((panels||[]).map(p => [p.id, p]));
    const pass = id => (byId[id]?.status === 'PASS'); const ready = Math.round(readiness?.score || 0);
    const items = [
    ...
    const wrap = document.createElement('div'); wrap.style.marginTop = '10px'; wrap.innerHTML = `<h4>Checklist</h4><ul>${html}</ul>`;
  ...
  async function renderTechChart(sym) {
    try {
      ...
      const dates = hist.dates; const closes = hist.close;
      const ma20 = sma(closes, 20); const ma50 = sma(closes, 50);
      const chartEl = el('tech-chart'); if (!chartEl || typeof LightweightCharts === 'undefined') return;
      chartEl.innerHTML = '';
  ...
  async function fetchMacro() {
    const text = el('macro-text')?.value; if (!text) return;
    setStatus('Analyzing macro...', 'loading');
    const res = await fetch('/api/macro-signal', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }),
    });
    if (!res.ok) { setStatus('Macro analysis failed', 'error'); return; }
    const data = await res.json(); const output = el('macro-output'); if (output) output.textContent = data.outlook;
  ...
  async function fetchRecs() {
    ...
    if (!res.ok) { setStatus('Failed to load recommendations', 'error'); return; }
    const data = await res.json();
    const body = document.querySelector('#recs-table tbody'); if (!body) return;
    body.innerHTML = '';
  ...
  async function fetchSingle() {
    const sym = el('symbol')?.value.trim().toUpperCase(); if (!sym) return;
    const res = await fetch(`/api/recommendation/${encodeURIComponent(sym)}`);
    if (!res.ok) return;
    const data = await res.json();
    const body = document.querySelector('#single-rec tbody'); if (!body || !data.recommendation) return;
    body.innerHTML = '';
  ...
  function runSignal() { Promise.allSettled([fetchSignal(), fetchSingle()]); }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adminLink && localStorage.getItem('isAdmin') === '1') adminLink.style.display = 'block';
    ...
    if (symbolInput) symbolInput.addEventListener('input', (e) => { e.target.value = e.target.value.toUpperCase(); });
    if (modeSelect) modeSelect.addEventListener('change', () => runSignal());
    runSignal();
  });
})();
