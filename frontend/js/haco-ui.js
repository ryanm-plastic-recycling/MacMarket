...
async function fetchHaco() {
  ...
  // Prefer full series (has Zl* fields for indicator lines)
  if (Array.isArray(data.series)) {
    renderChart(data.series);
    explainLast(data.last || {});
  } else if (Array.isArray(data.candles)) {
    // Fallback: convert candles->minimal bars so something still shows
    const bars = data.candles.map(c => ({ time: c.time, o:c.open, h:c.high, l:c.low, c:c.close }));
    renderChart(bars); // no indicator lines in this mode
    explainLast(null);
  } else {
...
// Wire up HACO "Run" explicitly to fetchHaco (NOT window.HACO.runScan)
const runBtn = document.getElementById('haco-run');
if (runBtn) {
  runBtn.addEventListener('click', (e) => {
    e.preventDefault();
    console.log('[HACO:UI] Run → GET /api/signals/haco');
    fetchHaco();
  });
}
// Optional quick hooks (if present)
...
// Initial HACO render on load
console.log('[HACO:UI] Initial render with default symbol');
fetchHaco();
